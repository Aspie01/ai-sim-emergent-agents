...
| Per-agent language | `Inhabitant.language` | Transactional communication/lexical variation, post-birth parental exposure, maintenance | Communication, language/dialect/contact/intergenerational/lexical summaries, hash | Retained on living/dead agent until death cleanup | Enabled final hash, including enabled contact/intergenerational/lexical metadata |
| Language runtime | `SimulationState.language` | Initialization, communication/lexical variation, post-birth exposure, maintenance | Validation, summaries, hash | In-memory run only | Enabled final hash and manifest controls |
...
| Lexical-evolution runtime | `SimulationState.lexical_evolution` | Eligible committed-transfer communication | Validation, on-demand summary, hash, reset | In-memory run only | Enabled final hash; controls in manifest; no standard lexical artifact |
...
- Lexical provenance is association-owned direct-edge metadata, not a global
  lineage graph. The dedicated runtime owns its future-affecting derivation
  index and bounded counters. Historical source IDs may resolve through the
  retained dead cohort; ordinary mutation and summaries perform no owner
  lookup.
