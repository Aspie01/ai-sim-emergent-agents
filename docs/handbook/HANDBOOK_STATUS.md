...
| Attempt ledger | Written under real execution. Every cell directory now carries `attempts.jsonl` recording the attempt start, its outcome, and a selection only when the artifacts deeply validate -- exit status alone never selects. Phase 1 of `V2_RUNNER_INTEGRATION_PLAN.md`: the layout is unchanged, `directory` records `.`, and nothing reads the ledger yet. Immutable attempt directories and resume remain Planned, not implemented |
