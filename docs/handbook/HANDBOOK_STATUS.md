...
| Resume into a populated output root | Live, and the only path on which a nonempty root is accepted. `--resume` is granted only when the root records a resume contract that matches in every field, the revision is clean, and every cell carries an attempt ledger; anything else refuses without touching a byte. A resumed cell gets a new attempt directory, its ledger is appended to, and the prior batch manifest is archived rather than overwritten. Overwrite remains refused outright. Phase 3 of `V2_RUNNER_INTEGRATION_PLAN.md`: cells are not yet skipped, so a resume re-attempts every cell |
| Resume contract and frozen schedule | Recorded, not yet acted on. Every batch manifest carries a `resume_contract` block with the exact fields `safe_resume.ResumeContract` compares, a `config_fingerprint` over per-cell controls that is independent of dispatch order, and a `schedule` block with the schedule id, planned order, and the actual dispatch order with wall-clock stamps. Phase 0 of `V2_RUNNER_INTEGRATION_PLAN.md`; the runner still refuses every nonempty root |
| Contract-matched safe resume | Decision logic implemented as machinery, **not wired into execution**. `src/thalren_vale/safe_resume.py` decides per cell whether a resume may skip work, refusing outright on any contract mismatch, a dirty tree, or evidence for cells the plan does not define, and skipping only a selected attempt that completed and deeply validates. The runner still refuses every nonempty output root, so no resume can execute yet |
| Fail-fast dispatch | Implemented, opt-in. `fail_fast` in a plan, `--fail-fast`, or the `run_from_plan` argument stops dispatch at the first cell that does not complete, persisting the stop reason, dispatch position, and undispatched count to the batch manifest before the loop breaks. A truncated batch is never marked complete. Opt-in so existing engineering batches keep running every cell |
| Nonexecuting matrix expansion | Implemented. `run_experiments.py --expand` prints the exact cells, dispatch order, child commands, output paths, per-cell ticks and timeouts, and the declared revision contract, then exits without creating a directory or starting a process. Expansion and execution share one cell builder so the listing cannot drift from what is dispatched |
| Attempt ledger and immutable attempt directories | Live. Each cell holds `attempts.jsonl` and each run executes into its own `attempt_NNNN/`, so a retry cannot overwrite a prior attempt's manifest, summary, or stderr; the allocator refuses an existing path rather than reusing it. Selection requires deep validation, not exit status. Readers resolve through `resolve_cell_artifacts`, which returns the selected attempt's directory, the cell itself for runs predating attempts, or a sentinel when a ledger exists with nothing selected -- that third case is reported unvalidatable and never falls back to reading the cell. Phases 1-2 of `V2_RUNNER_INTEGRATION_PLAN.md`; resume remains Planned, not implemented |
| Revision preflight | Implemented, opt-in. A plan declaring `expected_commit`, `expected_tag`, or `require_clean_revision` has it enforced before any output root is created, and an unreadable revision fails closed. Enforcement is opt-in because the runner also serves engineering characterization on untagged revisions |
| Environment and plugin preflight | Implemented, opt-in. `expected_environment_fingerprint` and `require_empty_plugin_inventory` are enforced before any output root is created. Every batch manifest now records the fingerprint, interpreter version and absolute executable, platform, the `PYTHONHASHSEED` policy, and the full plugin inventory including an explicit empty one. Dependency and lockfile hashes remain **not** recorded: `environment_fingerprint` deliberately excludes installed distributions because packaging metadata varies by install method |
