...
| `tests/test_safe_resume.py` | Contract-matched resume decisions: refusal on any contract mismatch, dirty tree, or unknown cell; skipping only a selected, completed, deeply valid attempt | Runner integration: the decision is machinery, and the runner still refuses every nonempty root |
